...
> Quick cross-reference: This root guide covers repo-wide operations, stabilization, and CI expectations. For service-level coding norms inside the `pmoves/` subtree (Python services, contracts, compose), also read `pmoves/AGENTS.md` and keep both in sync.
...
- Notebook Workbench: http://localhost:4482/notebook-workbench (run `npm run dev` inside `pmoves/ui`; default is single‑env mode — `pmoves/env.shared` is the source of truth; lint + REST check via `make -C pmoves notebook-workbench-smoke`).
