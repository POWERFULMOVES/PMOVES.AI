...
**HF MCP Server** `:8203` (host) / `:8096` (container) — HuggingFace Hub MCP server. Tools: `hf.model.search/info/download/list/convert_gguf`. SSE MCP at `/sse`, REST API at `/api/*`, publishes `hf.model.downloaded.v1`. Shares `${HF_HOME:-./data/models}`:/models with Ollama/vLLM. Health: `GET /healthz`. Profile: `agents`/`research`.
