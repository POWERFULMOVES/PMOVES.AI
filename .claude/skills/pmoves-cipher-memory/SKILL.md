...
## Important Submodules to Remember

### PMOVES-BoTZ
```
Category: submodule
"PMOVES-BoTZ: Multi-agent MCP platform with:
- Docling: Document processing (port 3020)
- Cipher: Memory management (port 8081)
- E2B: Code execution sandbox (port 7071)
- VL Sentinel: Vision-language processing (port 7072)
- Nested submodules: 15+ in features/ and skills/"
Tags: botz, mcp, multi_agent
```

### PMOVES-Archon
```
Category: submodule
"PMOVES-Archon: Knowledge management system with:
- Frontend: React + TanStack Query + Vite
- Backend: FastAPI + Supabase + PydanticAI
- MCP Server: Port 8051
- Agents: Port 8052
- Features: Projects, tasks, knowledge, progress tracking"
Tags: archon, knowledge, fastapi
```

### PMOVES-DoX
```
Category: submodule
"PMOVES-DoX: Document processing with:
- Nested Agent-Zero: Separate instance for DoX-specific branch
- Nested references: A2UI, PsyFeR, Glancer, conductor
- Integration: Runtime networking to parent PMOVES.AI
- Not a circular reference - separate instances!"
Tags: dox, document_processing, nested
```
...
1. **Be Specific**: "PMOVES-BoTZ uses pmoves_integrations" vs "uses framework"
2. **Include Context**: Why was this decision made?
3. **Link Related**: Use tags to connect related memories
4. **Update Over Time**: Edit memories when patterns evolve
5. **Cross-Reference**: Mention related submodules and services

---

**Remember**: This memory persists across all sessions. Invest in building a knowledge base that speeds up future PMOVES.AI development.
