...
by that twice in one week:
  ...
  * #3152 bumped the pin — `c88b009a2` -> `7ac00b1b` (fork PR #27, per-request
    MCP identity + CIPHER_MCP_ENFORCE). It touches `mcp-sse.ts`,
    `rest-server.ts` and the pmoves README, not `auth.ts`; all nine citations
    re-verified at the same lines. NOTE: `7ac00b1b` is the head of an UNMERGED
...
PIN = "7ac00b1bd4099b7a8edb0afadb2b58b7656f57b5"
