...
def test_no_cli_references_in_compose() -> None:
    ...
    result = subprocess.run(
        ["grep", "-r", "54321", "/home/pmoves/PMOVES.AI/pmoves/docker-compose*.yml"],
        capture_output=True,
        text=True,
    )
...
def test_env_uses_selfhosted_urls() -> None:
    ...
    result = subprocess.run(
        ["grep", "SUPABASE_REST_URL", "/home/pmoves/PMOVES.AI/pmoves/env.shared"],
        capture_output=True,
        text=True,
    )
...
def test_supabase_tenant_configured() -> None:
    ...
    result = subprocess.run(
        ["grep", "-A", "5", "supabase-realtime:", "/home/pmoves/PMOVES.AI/pmoves/docker-compose.yml"],
        capture_output=True,
        text=True,
    )
...
def test_env_shared_has_jwt_comment() -> None:
    ...
    result = subprocess.run(
        ["grep", "-A", "2", "SUPABASE_JWT_SECRET", "/home/pmoves/PMOVES.AI/pmoves/env.shared"],
        capture_output=True,
        text=True,
    )
...
def test_no_duplicate_jwt_secrets() -> None:
    ...
    result = subprocess.run(
        ["grep", "^SUPABASE_JWT_SECRET=", "/home/pmoves/PMOVES.AI/pmoves/env.shared"],
        capture_output=True,
        text=True,
    )
...
def test_supabase_env_file_exists() -> None:
    ...
    result = subprocess.run(
        ["test", "-f", "/home/pmoves/PMOVES.AI/pmoves/env.tier-supabase"],
        capture_output=True,
    )
    ...
    for var in required_vars:
        result = subprocess.run(
            ["grep", f"^{var}=", "/home/pmoves/PMOVES.AI/pmoves/env.tier-supabase"],
            capture_output=True,
            text=True,
        )
