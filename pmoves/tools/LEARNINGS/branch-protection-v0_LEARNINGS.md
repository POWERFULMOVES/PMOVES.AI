# Branch protection v0 - LEARNINGS (5-class taxonomy + 6 pair-review lessons + ownership split)
...
2. `required_signatures` and `require_linear_history` are **ruleset rules**, not classic fields
...
8. **The spec's `~DEFAULT_BRANCH` sentinel in `conditions.ref_name.include` is a contract with the audit, not a literal value.** The spec writes `"include": ["~DEFAULT_BRANCH"]` to mean "the repo's actual default branch" (which the API returns as the literal name, e.g. `"main"`). The deep diff cannot compare them without knowing the default branch. The right behavior: skip the include check entirely when the spec uses the sentinel (the spec author already declared the target as "the default branch" by using it). The "fix" is a one-line `pass` (vs. running the comparison) and a clear code comment, plus a regression test. Test: `pmoves/tools/tests/test_branch_protection.py::RulesetDiffTests::test_D7_diff_skips_include_check_when_spec_uses_default_branch_sentinel`.

## Ratification: 5 of 6 P1s collapse to deletions (N1/N2/N3/P1-A/P1-C/N8)
...
| **N2** | Migration's signed commits + linear history silently dropped | **DELETED** with the migration — `required_signatures` + `require_linear_history` are now expressible as ruleset rules on PMOVES.AI's `[ main ]` ruleset; the operator's escape hatch is preserved via the spec's `bypass_actors` |
