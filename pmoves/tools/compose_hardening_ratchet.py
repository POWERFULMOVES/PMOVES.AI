...
def write_baseline(idents: List[str], path: Path = BASELINE) -> None:
    ...
    lines = [
        ...
        "#",
        "# Each entry is a (property, service) pair for a service declared in",
        "# pmoves/docker-compose.hardened.yml and resolved against the base",
        "# stack it is deployed on, that does not meet the hardening property",
        "# named. They are recorded so the check can be ENFORCED today without",
        "# turning the gate red in a single step. They are NOT approved, and",
        "# none of them is 'expected'.",
        "#",
        "# NOTE: --write-baseline regenerates this header and DROPS any prose",
        "# added by hand. The 2026-09-21 entry explaining why 28 false",
        "# NO_RESOURCE_LIMITS entries were removed lives in git history if a",
        "# regeneration has eaten it.",
        "#",
