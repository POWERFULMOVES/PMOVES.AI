...
| R9700 Workstation (Linux) — alias **B850 "Knuckles"** | LAN | `ts:<rdna4>` (target) — current `100.122.182.3` | pmoves-rdna4 (target); **current `pmoves-b850-ai-top`** | Heavyweight ROCm Inference (Gemma 4 31B/26B-A4B via llama.cpp). **Current state (2026-05-16): 1× R9700 detected (PCI dev `7551`), 30GB RAM, Ubuntu 24.04.4, /dev/kfd present, ROCm install operator-pending Phase-C.** Box hosts the Claude Code dev shell as `cataclysmstudios@pmoves-b850-ai-top`. | `self-hosted, ai-lab, gpu, rocm, rdna4` (pending) | 9850X3D / 30GB + 1× R9700 currently (target: 2× R9700 / 64GB VRAM total per Phase-C) | electricity |
