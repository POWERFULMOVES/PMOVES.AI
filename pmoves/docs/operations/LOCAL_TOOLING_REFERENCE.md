...
### TensorZero & Ollama
- `make up-tensorzero` now starts ClickHouse, the TensorZero gateway/UI, and an Ollama sidecar automatically so local embeddings (default `qwen3-embedding:4b`) are ready to use.
...
- `make notebook-up` → launches the optional Open Notebook research workspace (Next.js UI on `${OPEN_NOTEBOOK_UI_PORT:-8503}`, REST API on `${OPEN_NOTEBOOK_API_PORT:-5055}`). Pair with `make notebook-logs` for tailing output and `make notebook-down` to stop it without removing data under `pmoves/data/open-notebook/`. Once `env.shared` has your API token/password and provider keys, run `make notebook-seed-models` to auto-register the default model catalogue upstream so the UI can save settings without manual SurrealDB edits.
...
- Extract worker embeddings: set `EMBEDDING_BACKEND=tensorzero` (default `sentence-transformers`) to route chunk embeddings through the TensorZero gateway, honouring `TENSORZERO_BASE_URL` and `TENSORZERO_EMBED_MODEL` (`tensorzero::embedding_model_name::qwen3_embedding_4b_local`, backed by Ollama's `qwen3-embedding:4b`). Restart `extract-worker` after changes.
...
- `make -C pmoves danger-room-build-theme` → play the Danger Room build theme. Audio source order:
  1. `DANGER_ROOM_THEME_URL` (HTTP(S) URL or local file path) if set.
  2. Flute Gateway TTS using `DANGER_ROOM_THEME_TEXT`, `DANGER_ROOM_THEME_PROVIDER`, `DANGER_ROOM_THEME_ENGINE` (defaults: `ultimate_tts` / `kitten_tts`).
  3. Fallback spoken announcement if the gateway is unreachable.
- `make -C pmoves danger-room-build IMAGE=agent-zero` → wrap `ghcr-build-one` with theme fanfare before/after the build. Best-effort: a missing audio player or gateway will not fail the build.
...
## Verification & Smokes
- `make smoke-wger` → runs HTTP checks against `http://localhost:8000` and `/static/images/logos/logo-font.svg` through the nginx sidecar so Wger matches the upstream static-serving deployment guidance.
