...
- `make channel-monitor-up` / `make channel-monitor-smoke` → start the YouTube channel monitor service and trigger a one-off RSS pass. Uses `CHANNEL_MONITOR_*` env vars defined in `.env`, `.env.local`, and `env.shared`. Set `CHANNEL_MONITOR_SECRET` (channel monitor) + `CHANNEL_MONITOR_STATUS_URL`/`CHANNEL_MONITOR_STATUS_SECRET` (pmoves-yt) so ingestion callbacks can mark rows `completed`/`failed` once downloads finish.
