...
- **Single‑env manifest (CHIT‑ready):** `SINGLE_ENV_MODE=1` (default) forces both Docker Compose and the UI launcher to source everything from `pmoves/env.shared` (plus `env.shared.generated`). This is the manifest we ship through CHIT and GitHub secrets—one file, checked into infra, consumable by every module. Override locally only when you deliberately export `SINGLE_ENV_MODE=0`.
- **Per‑service bring-up:** Each service folder ships its own `.env.example` (for example `pmoves/services/pmoves-yt/.env.example`, `pmoves/services/channel-monitor/.env.example`). They inherit shared secrets from `env.shared`, but can run standalone: each Make target or compose profile probes for dependencies (Supabase, NATS, etc.) and will spin a local copy only when the shared instance is missing. This keeps the modular architecture intact while giving the single-env file authoritative defaults.
- **CHIT secrets flow:** `pmoves/env.shared` stays the source of truth. `make env-setup` regenerates `env.shared.generated` + `.env.generated` for tooling, and `python -m pmoves.scripts.secrets_sync upload --include-optional` publishes the combined manifest to CHIT. Downstream containers and runners fetch the same bundle so dev, CI, and production read matching values. Keep service-specific examples in sync so they map 1:1 with the shared keys you publish.
...
### TensorZero & Ollama
- `make up-tensorzero` now starts ClickHouse, the TensorZero gateway/UI, and an Ollama sidecar automatically so local embeddings (e.g., `embeddinggemma:300m`) are ready to use.
- To target a remote TensorZero instead of local, set `TENSORZERO_BASE_URL=http://<remote>:3000` (in the shell or `.env.local`). Gateways read `EMBEDDING_BACKEND` + `TENSORZERO_*`/`OLLAMA_*` URL vars and fall back to sentence-transformers if providers are unavailable.
...
- `scripts/install_all_requirements*.{sh,ps1}` → one-shot installs for every Python requirement file when you need parity with CI or remote hosts.
  - Optional TensorZero gateway: copy `pmoves/tensorzero/config/tensorzero.toml.example` to `pmoves/tensorzero/config/tensorzero.toml`, then set `TENSORZERO_BASE_URL=http://localhost:3030` (and `TENSORZERO_API_KEY` if the gateway enforces auth). `make -C pmoves up-tensorzero` now boots ClickHouse, the gateway/UI, and `pmoves-ollama`; point `TENSORZERO_BASE_URL` at a remote gateway if Ollama runs elsewhere. LangExtract will honour `LANGEXTRACT_PROVIDER=tensorzero` and forward `LANGEXTRACT_REQUEST_ID` / `LANGEXTRACT_FEEDBACK_*` metadata tags when these variables are present.
...
- TensorZero gateway (optional): `make -C pmoves up-tensorzero` (or `docker compose --profile tensorzero up tensorzero-clickhouse tensorzero-gateway tensorzero-ui pmoves-ollama`) boots the ClickHouse backing store, gateway/UI, and Ollama sidecar (http://localhost:4000). Point `TENSORZERO_BASE_URL` at `http://localhost:3030` so LangExtract and the Crush configurator detect the gateway automatically, or redirect to a remote gateway when inference runs off-box.
...
- Extract worker embeddings: set `EMBEDDING_BACKEND=tensorzero` (default `sentence-transformers`) to route chunk embeddings through the TensorZero gateway, honouring `TENSORZERO_BASE_URL` and `TENSORZERO_EMBED_MODEL` (`tensorzero::embedding_model_name::gemma_embed_local`, backed by Ollama's `embeddinggemma:300m`). Restart `extract-worker` after changes.
