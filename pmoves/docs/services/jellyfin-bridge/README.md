...
Environment
- `JELLYFIN_URL` (default `http://jellyfin-ai:8096`, host overlay exposed on `http://localhost:9096`)
- `JELLYFIN_API_KEY`
- `JELLYFIN_USER_ID`
...
Notes
- `make jellyfin-smoke` now handles the common “no library item yet” case by linking the latest Supabase video to the newest Jellyfin asset via `/jellyfin/link` before requesting `/jellyfin/playback-url`. Keep at least one item in the overlay libraries (`python scripts/seed_jellyfin_media.py`) so fallback linking succeeds.
