...
| Built from | `../PMOVES.YT`, `pmoves_yt_service/Dockerfile` — the **fork**, vendored yt-dlp tree | `pmoves/services/pmoves-yt/Dockerfile` — `pip install yt-dlp[default]`, **stock PyPI** |
| yt-dlp | the fork: CHIT signing, NATS publish, SoundCloud fixes, phase9c multi-client | stock upstream, **without** those commits |
...
**UNVERIFIED:** whether `ghcr.io/powerfulmoves/pmoves-yt:pmoves-latest` exists in the registry, and
what yt-dlp version it carries. `docker manifest inspect` returns `denied` anonymously and
`gh api users/POWERFULMOVES/packages/container/pmoves-yt/versions` returns
`403 ... needs at least read:packages scope`. An operator on the published path cannot confirm what
they are pulling without a `read:packages` token. Verify against `/healthz` after bring-up.
...
with `git checkout -- PMOVES.YT` (or `git submodule update --init --force PMOVES.YT`) and rebuild.
...
**Known papercut:** `yt-cookies-status` runs on the **host** but talks to `SUPABASE_URL`, which
defaults to the in-network hostname `supabase-kong`. From the host that does not resolve:
...
This is an environment mismatch, not a broken cookie pipeline. Either point `SUPABASE_URL` at a
host-reachable address for the invocation, or query the refresher's API directly on `:8115`.
...
# follow the whole chain at once (not blocked by the governance hook)
...
| `/healthz` reports a yt-dlp five months older than expected | Someone ran `git submodule update --remote` | §2.2 | `git checkout -- PMOVES.YT`, rebuild |
| An extractor bug you *know* is fixed in the fork is still present | Node is on the published path (stock PyPI yt-dlp) | §1.1 | Switch to `up-yt`, or fix the fork's Dockerfile in the GHCR matrix |
...
| `yt-cookies-status` → `httpx.ConnectError: Name or service not known` | Host-run target, in-network `SUPABASE_URL` | §5.4 | Env mismatch, not a pipeline fault |
...
```bash
# 1. Stop the YT chain, keeping containers, volumes and cookies intact (preferred)
docker compose -p pmoves stop pmoves-yt ffmpeg-whisper bgutil-pot-provider

# 2. Include the cookie pair
...
1. **The published GHCR image.** Existence and yt-dlp version of
   `ghcr.io/powerfulmoves/pmoves-yt:pmoves-latest` could not be confirmed — `docker manifest inspect`
   → `denied`; `gh api users/POWERFULMOVES/packages/container/pmoves-yt/versions` →
   `403 ... read:packages scope`. The *stock-PyPI* claim is verified from the Dockerfile the GHCR
   matrix builds (§1), not from the published artifact.
