...
- Tokenism settlement has typed requested/recorded/failed NATS contracts, a deterministic planner, Firefly and contract dry-run executors, live executor gates requiring signed executor identity plus matching signed operator approval, manifest-backed chain call drafts, and schema-validated signed result publishing for recorded/failed events.
...
Not done:
- Production settlement deployment review, RPC/wallet custody, and deployment-address attestation.
...
| `PMOVES-ToKenism-Multi/IMPLEMENTATION_STATUS.md` | Added scope reality check and updated schema/test/status claims, including guarded Firefly and contract executors. |
...
| `PMOVES-ToKenism-Multi/integrations/contracts/SETTLEMENT_FLOW.md` | Defines the signed settlement event flow, idempotency contract, Firefly/contract dry-run paths, signed result publishing, and live-execution gates. |

## Recommended Next Atomic Workstreams

1. **Production deployment attestation:** bind deployed contract addresses, RPC/wallet custody, FireFly environment, and operator approval records into signed deployment manifests before enabling production settlement.
