...
Not done:
- Production token settlement through NATS -> FireFly -> contract flow.
...
| `PMOVES-ToKenism-Multi/PHASE4_*.md` and `INTEGRATED_EXECUTION_PLAN.md` | Clarified that Firefly modules are complete but production ingestion/settlement remains pending. |

## Recommended Next Atomic Workstreams

1. **Tokenism settlement design:** define the NATS -> FireFly -> contract flow, payload schemas, idempotency keys, and failure handling before code.
