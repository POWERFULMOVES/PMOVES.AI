...
## Wger profile
- `pmoves/compose/docker-compose.wger.yml` provisions a Postgres 15 database plus the PMOVES-tagged `ghcr.io/cataclysm-studios-inc/pmoves-health-wger:pmoves-latest` image behind the `wger` compose profile and binds the UI to `8000/tcp`.【F:pmoves/compose/docker-compose.wger.yml†L1-L27】
