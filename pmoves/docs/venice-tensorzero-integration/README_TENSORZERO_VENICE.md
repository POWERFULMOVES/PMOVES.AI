...
- If you’re working inside this repository, you don’t need to copy files around. Use:
  ...
  - `make -C pmoves model-apply PROFILE=archon HOST=desktop-9950xd` to write env values into `pmoves/.env.local`.
...
- Run `make -C pmoves models-sync PROFILE=agent-zero HOST=desktop-9950xd` (or `archon`, `media`, etc.) to apply the curated manifests via `pmoves/tools/models/models_sync.py`. Profiles capture provider-specific knobs (Ollama, vLLM, llama.cpp, Jetson builds) so multi-arch deployments get the right context window, decoding, and endpoint wiring.
