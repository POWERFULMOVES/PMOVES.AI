...
### Keeping the Supabase CLI Updated (no tarballs required)

- Run `make supa-cli-update` to stop the local stack and invoke the right package manager automatically; the helper falls back to manual instructions if no supported manager is detected.
- **Pause local services first**: run `supabase status` to confirm what is running, then `supabase stop` (plus `make down` if Compose services are active). Capture local changes with `supabase db diff` / `supabase db dump` before stopping if you have work-in-progress data.
- **Windows (Scoop install)**: open PowerShell and run `scoop update supabase`.
- **Windows (winget)**: `winget upgrade Supabase.Supabase` if you originally installed via winget.
