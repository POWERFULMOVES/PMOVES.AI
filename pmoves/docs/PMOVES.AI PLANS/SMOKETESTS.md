...
> **Prereq:** Populate `DISCORD_WEBHOOK_URL` (and optional username/avatar overrides) in `env.shared` or `.env.local`. The shared `.env` file keeps these keys commented so a blank assignment does not override your secrets.
...
2. Optional signing (if `CHIT_REQUIRE_SIGNATURE=true` in gateway):
    ...
    doc = json.load(open('cgp.json', 'r', encoding='utf-8'))
    signed = sign_cgp(doc['data'], 'change-me')
    json.dump({'type': 'geometry.cgp.v1', 'data': signed}, open('cgp.signed.json', 'w', encoding='utf-8'))
    PY
    ```
...
2. Open [http://localhost:8086/geometry/](http://localhost:8086/geometry/) (use [http://localhost:8087/geometry/](http://localhost:8087/geometry/) if you started the GPU profile)
...
Both targets auto-install `tools/flightcheck/requirements.txt` via uv/pip before running the retro checks.
