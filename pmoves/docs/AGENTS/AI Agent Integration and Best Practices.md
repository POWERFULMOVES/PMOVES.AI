...
4. **F-Thread (Fusion):** A quality assurance pattern where the same prompt is sent to multiple disparate models (routed via TensorZero). The system then "fuses" the results, selecting the best components or synthesizing a consensus solution to minimize hallucinations.3  
