...
|---|---|---|---|
| L1 — Roster | ZaClaw into `agent_registry.yaml` + `agent_signatures.yaml` + register CLAIM row (TTL) | `make -C pmoves validate-agents` **plus a signature check that does not exist yet** — `validate_agent_registry.py` reads registry, teams, vocabulary and rooms and has ZERO references to `agent_signatures.yaml`, so it cannot verify glyph uniqueness, WCAG colour contrast or `co_author` shape; an invalid signature passes it. Add that gate as part of L1. PR off `origin/main` (NOT the wip snapshot branch) | OPEN — blocked on identity decisions |
| L2 — DARKXSIDE digital-twin corpus | Playlist = content source + twin dataset; ingestion/classification **already built** (2,028 crawled / 2,017 classified, 11 resonance domains); ZaClaw adds distillation + wiring, not re-ingestion | Persona suite 06/07/08/09, `yt_playlist_enrich.py` + Supabase `youtube_videos`, persona room PRs #2236–#2246, `make -C pmoves persona-render` | OPEN — stats pagination fix, persona.json sync, auto-research loop, ActivePieces flow, persona.pmoves.ai DNS (verified still absent 2026-08-27) |
| L6 — Aragon peer-corpus review | Review Richard Aragon's latest work (**Dendriton** + videos; also new Discover AI + IndyDevDan videos) → combine with the memory/RAG graph layer (Cipher, HiRAG, Qdrant, Neo4j). His math already seeded CHIT — attribution carried forward | `youtube-watcher` + transcript rails; `research/` analysis-doc format; Cipher (`:8105/mcp/sse`), HiRAG, Qdrant, Neo4j services | OPEN — Dendriton has zero refs in repo (net-new); @richardaragon8471 not wired. **Discover AI is already monitored** — `pmoves/config/channel_monitor.json` carries it as `yt:channel:@code4AI` (`channel_name: Code4AI`) with its own archive and `pmoves.youtube.ai` namespace; adding it again would duplicate polling and ingestion. Only the Aragon wiring is missing |
| L3 — Hostinger VPS migration | Move claw runtime to Hostinger VPS; register node in `fleet-map.yaml`; secrets via CHIT funnel; **candidate host for the persona-room public edge** (07 §4); **port other Hostinger-hosted sites to Cloudflare** — account already runs `pmoves-ai` + `cataclysmstudios` Pages projects, zero Workers (verified 2026-08-27 via connector) | `pmoves/config/mcp/hostinger.yaml` (Terraform+SSH, `make -C pmoves hostinger-provision`), `deploy/sidecar/`, `vps_fleet_manager` (`mesh.vps.*` subjects), Cloudflare Pages/DNS via connector | OPEN |
...
| L9 — Composio MCP | Composio hosted MCP gateway (SaaS toolkits) — **operator-approved 2026-08-27**; inventory entry lands in **PR #2795**, not this one, and is scoped `clients: []` rather than merely `disabled: true` — `disabled` is a Cline/Roo key absent from Claude Code's schema, so it does NOT keep the server out of a generated Claude config (measured). Activation = operator funnels toolkit URL + `COMPOSIO_API_KEY`, registers the key in the canonical CHIT manifest and client launcher allowlists, verifies transport/headers, then populates `clients` | `pmoves/config/mcp_inventory.json` (canonical → `mcp_config_generator.py`) | ENTRY IN #2795 (scoped `clients: []`, served to nobody) — awaiting creds |
...
- `docs/youtubelist.md`: the 13-channel roster. IndyDevDan wired (prior research at repo root: `Aligning AI Agents with Indy Dev Dan.md`). **Discover AI is already wired** as `@code4AI` in `pmoves/config/channel_monitor.json` — an earlier draft called it unmonitored; that was wrong.
