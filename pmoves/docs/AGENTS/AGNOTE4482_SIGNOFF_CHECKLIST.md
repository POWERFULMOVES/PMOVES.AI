...
| `CODEX-GPT5` | P7 room contract delivery | Guarded room schema/catalog, manifest parity, registry/team coupling, P7 stage/session runtime, CHIT activation semantics, NATS command/fact roles, Compose/port wiring, focused tests, and canonical docs | SIGNED | 2026-07-17 | Environment-backed NATS/Supabase gate verified with session `0db7c6ff-8e93-44a5-a1bb-3a749c314c34`; additional per-room operator activation remains outside this signature. |
...
| `EM-FLASH (hermes)` | identity registration + wave receipt | identity_vocabulary.yaml (em-flash identity, hermes harness entry, PMOVES-4090-CLAUDE umbrella alias), AGInTZ ledger row, SITREP 2026-09-24 addendum, this lane's CLAIM row in AGNOTE4482PHI.t1.md | SIGNED | 2026-09-24 | Docs-only lane; no code or guard changes. PMOVES-4090-CLAUDE alias is operator-declared pending the owner's own registration. |
...
| `CRUSH-SPARK` | delivery — 2026-09-08 merge wave | Five PRs: #2946 pair-review harness-agnostic (+5 P2 fixes), #2942 a0-archon-bridge skill (healthz runtime validation), #2980 H3 V3 combiner archive (GB10 stage-1/4 evidence), #2959 gh-app local mint (importorskip collection fix), #2943 archon-native-only (26 files, -1991, runbook caller migration, mcp_inventory union). All merged via guarded closeout (head-pinned, fail-closed audits, 5/5 required checks per PR, 0 unresolved threads). | SIGNED | 2026-09-08 | [ACK: delivery] Review findings addressed in-branch before merge; FlOO$ pr-monitor chain run (validate/resolve/dry + CHIT packet); strict gate exit-1 documented as fleet-wide drain (other lanes). |
| `OPERATOR` | control — 2026-09-08 merge wave | Directive "lets review for merge" then "ok to proceed" (session 2026-09-08); merge authorization for the five-PR wave with ADMIN_REVIEW_BYPASS (same-account fleet pattern, author-restricted to POWERFULMOVES). | SIGNED | 2026-09-08 | [ACK: control] DARKXSIDE authorization recorded in-session; standing decisions still open: 4TB split, GH_APP_SEC PEM upload. |
| `CRUSH-SPARK` | memory — 2026-09-08 merge wave | Session checkpoints under agentId crush-spark (cipher): H3 stage-1..4 substrate + incidents (hf-partial-loss on SIGTERM, torch cu128 clobber pattern, ComfyUI model-dir layout, UI-vs-API format gap, gh 2.45 to 2.100 tooling fix, GraphQL fleet-budget race + window-watcher merge pattern). | SIGNED | 2026-09-08 | [ACK: memory] CHIT trail signed via make sign-trail in the carrier; learnings packet at pmoves/docs/logs/pr_monitor_learnings_latest.cgp.json. |
