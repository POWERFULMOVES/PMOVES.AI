...
|---|-------|---------------------|----------------|------------|
| 1 | **contract-correctness** | The lane ledger (`pmoves/data/chit/lanes.jsonl`) MUST append BEFORE the subprocess runs — the audit trail exists even if the chit tool fails. The test `test_failed_subprocess_still_appends` pins this contract by deliberately failing the subprocess and asserting the ledger still got the entry. | `pmoves/tools/cipher_cli.py:133` (`cmd_register`) calls `_append_lane(...)` before `_run_chit(...)`. The test name asserts the POSITIVE behavior it pins; an earlier revision was named `test_failed_subprocess_does_not_append`, which asserted the opposite of what it tested (renamed in PR #3093 review). | Any audit-ledger-write pattern where the underlying action can fail independently of the audit. |
| 2 | **contract-correctness** | 4 narrowly-defined exit codes (0 ok, 2 usage, 3 wrapped tool failed, 4 health endpoint down), NOT the generic Unix 1. Callers branch without false positives — a chit tool that prints stderr warnings but returns 0 stays a 0 here. Exit 3 (lane failed) and exit 4 (cipher API down) carry different operator alerts: exit 3 = "the agent's task failed", exit 4 = "the agent is deaf to incoming CHIT messages". Wrapped-tool rc ≥ 2 passes through verbatim (its stderr already reached the terminal, subprocess.run does not capture it); only rc 1 is remapped to 3 via `_map_wrapped_rc`. | `pmoves/tools/cipher_cli.py` docstring "Exit codes" section + `_map_wrapped_rc` + each exit site. | Any CLI that wraps subprocess tools and needs to be monitorable. |
...
| 7 | **contract-correctness** | The lane ledger lives at `pmoves/data/chit/lanes.jsonl` which is GITIGNORED wholesale (pmoves/.gitignore — `data/chit/` directory). An earlier revision of the cipher_cli.py ledger comment CLAIMED the ledger "SHOULD be in git" — that was wrong and is now fixed (PR #3093 review): the ledger is intentionally local-node audit data, and entries embed host/user that must not reach a public repo. Lane history belongs in cipher MCP / lane-aware AGNOTE rows (like this one), NOT in committed JSONL files. | `pmoves/.gitignore` (`data/chit/`); the ledger comment in `pmoves/tools/cipher_cli.py`. | Any module docstring that asserts a git-trackability claim — verify it against `.gitignore` before claiming. |
