...
7. **Seeded-live rooms bypass the CHIT gate.** Only 3 of 13 manifests carry `meta.chit.card_id`; `persona.room.livingdoc.json` ships `stage: live` with no card, and three rooms carry `current_stage: live` sourced from `"ROOMS_ON_A_STAGE.md typical-stage table"` — a documentation table, not an activation event. `transition.py:183` gates only API-driven rehearsal→live. **Re-gate on restart, or accept that 'live' means two different things?**
   ...
   **Resolved 2026-09-08 (4090-CLAUDE-OPUS-5, field).** Re-gated. "Bypass" understated it: `transition.py` returns `noop: True` when `current_stage == target_stage`, and that return happens *before* the `rehearsal → live` branch — so `check_chit_activation` was not merely skipped for those three rooms, it was unreachable. The cause is a substituted source, not a bad judgement call: the Open Room lane CLAIM (2026-07-20, § Open Room Lane in the active claim register) specified `current_stage` be **read from each manifest's `stage` field**; the RELEASE filed the same day shipped "3 live / 5 rehearsal / 1 archive" against manifests that read 1 live / 12 rehearsal, recording the source as the `ROOMS_ON_A_STAGE.md` typical-stage table. Nobody read the release back against the claim, so it stood for 50 days. The three rows are reseeded to the stage their manifests declare, and `make -C pmoves room-catalog-stage-check` now refuses any `current_stage` that is neither a P7 writeback receipt nor equal to its manifest. **Left open deliberately:** `persona.room.livingdoc` still ships `stage: live` with no CHIT card. That is the other half of this item — a manifest declaring its own stage, rather than the catalog being seeded — and it is a policy question about a room this lane does not own.
