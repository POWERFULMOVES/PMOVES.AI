...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY read_anchors_tenant ON public.anchors FOR SELECT USING (namespace = current_setting(''app.current_tenant'', true) OR namespace = ''pmoves'')';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='constellations' AND policyname='read_constellations_tenant'
  ) THEN
    EXECUTE 'CREATE POLICY read_constellations_tenant ON public.constellations FOR SELECT USING (namespace = current_setting(''app.current_tenant'', true) OR namespace = ''pmoves'')';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='shape_points' AND policyname='read_shape_points_tenant'
  ) THEN
    EXECUTE 'CREATE POLICY read_shape_points_tenant ON public.shape_points FOR SELECT USING (namespace = current_setting(''app.current_tenant'', true) OR namespace = ''pmoves'')';
  END IF;
  IF NOT EXISTS (
