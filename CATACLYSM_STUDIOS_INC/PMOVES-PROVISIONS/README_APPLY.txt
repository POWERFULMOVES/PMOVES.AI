...
This staged copy ships with the portable provisioning bundle generated by
`python3 -m pmoves.tools.mini_cli bootstrap`. It mirrors the canonical
instructions stored in `docs/provisioning/README_APPLY.txt` so remote
recipients have the same guidance without browsing the full repository.
