...
## Hardware Notes
- **Jellyfin 10.11 requires 64-bit hosts.** Only x86_64 and aarch64 targets are supported going forward. Legacy 32-bit Raspberry Pi nodes (Pi 3-era) should be retired.
- **Replacement guidance:** see `inventory/nodes.yaml` for the current provisioning inventory. Recommended upgrades include Raspberry Pi 5 (8GB), Jetson Orin Nano 16GB, or x86_64 mini PCs/Nucs depending on workload.
- **Upgrade lint:** run `make manifest-audit` (new in this bundle) before upgrades to flag unsupported hardware in the manifest or Supabase deployment exports.
