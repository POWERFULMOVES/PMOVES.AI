...
# install-codex-pmoves.ps1
# ---------------------------------------------------------------------------
# Makes `codex-pmoves` a bare command in PowerShell on THIS node,
# so you can type `codex-pmoves` from any directory instead of the
# full launcher path.  Mirrors install-claude-pmoves-command.ps1 exactly.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File deploy\provision\install-codex-pmoves.ps1
#
...
$cmd = Join-Path $repoRoot "deploy\provision\codex-pmoves.cmd"
...
$block = @"
$begin
# Auto-generated by install-codex-pmoves.ps1.
# Purpose: Codex CLI rooted in the PMOVES repo
function codex-pmoves { & "$cmd" @args }
...
if (Test-Path $shimDir) {
    $shim = Join-Path $shimDir 'codex-pmoves.cmd'
    $q = [char]34
...
Write-Host ''
Write-Host "Installed 'codex-pmoves' -> $cmd"
