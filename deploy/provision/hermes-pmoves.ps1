...
# hermes-pmoves.ps1 -- launch `hermes` with pmoves/env.shared loaded
# ===========================================================================
# Auto-generated PowerShell twin of hermes-pmoves.sh. Same shell
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File deploy\provision\hermes-pmoves.ps1 [args]
#   .\hermes-pmoves.cmd                     # Windows Explorer double-click
#   install-hermes-pmoves-pmoves-command.ps1 ; hermes-pmoves     # profile function
#
...
if (Test-Path $envf) {
    ...
    # These control the hermes SDK itself, not MCP creds -- sourcing them would
    ...
    if (Test-Path $mavis_helper) {
        . $mavis_helper
        Strip-MavisSdkEnvFor -CliName 'hermes'
    } else {
        Write-Warning "[hermes-pmoves] mavis_sdk_env.ps1 not found at $mavis_helper -- Mavis SDK env may bleed into the launched session."
    }
    ...
    Write-Host "[hermes-pmoves] loaded $n vars from $envf"
    $script:launcherSession = "hermes-pmoves.ps1 ($n vars)"
} else {
    Write-Warning "[hermes-pmoves] $envf not found -- MCP creds may be missing."
    $script:launcherSession = "hermes-pmoves.ps1 (env file NOT FOUND)"
}
