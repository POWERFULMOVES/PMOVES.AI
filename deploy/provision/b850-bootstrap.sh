...
preflight_pmoves_dir() {
  ...
  if pmoves_dir_is_legacy; then
    cat >&2 <<EOF
[b850-bootstrap] ERROR: ${PMOVES_DIR} has the LEGACY pre-checkout layout
[b850-bootstrap]        (bin/ data/ etc/ lib/ logs/ profile.yaml, no .git).
[b850-bootstrap]        This script now expects ${PMOVES_DIR} to be a PMOVES.AI git
[b850-bootstrap]        checkout owned by ${OPERATOR_USER}. Nothing has been changed.
[b850-bootstrap]
[b850-bootstrap] Migrate (moves, never deletes), then re-run:
[b850-bootstrap]   sudo mv ${PMOVES_DIR} ${PMOVES_DIR}.legacy-${ts}
[b850-bootstrap]   sudo bash $0
[b850-bootstrap] The re-run clones the checkout and writes a fresh profile.yaml.
[b850-bootstrap] To keep the OLD profile instead, after the re-run:
[b850-bootstrap]   sudo install -m 0644 -o ${OPERATOR_USER} -g ${OPERATOR_GROUP} ${PMOVES_DIR}.legacy-${ts}/profile.yaml ${PMOVES_DIR}/profile.yaml
[b850-bootstrap] Check ${PMOVES_DIR}.legacy-${ts}/data before removing the legacy tree.
EOF
  else
    cat >&2 <<EOF
[b850-bootstrap] ERROR: ${PMOVES_DIR} exists, is not empty, and is not a git checkout
[b850-bootstrap]        (and is not the known legacy layout). Nothing has been changed.
[b850-bootstrap] Move it aside and re-run, e.g.:
[b850-bootstrap]   sudo mv ${PMOVES_DIR} ${PMOVES_DIR}.aside-${ts}
[b850-bootstrap]   sudo bash $0
[b850-bootstrap] or point the bootstrap elsewhere: PMOVES_DIR=/path sudo -E bash $0
EOF
  fi
  exit 1
}
