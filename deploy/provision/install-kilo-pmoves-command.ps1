...
# install-kilo-pmoves.ps1
# ---------------------------------------------------------------------------
# Makes `kilo-pmoves` a bare command in PowerShell on THIS node,
# so you can type `kilo-pmoves` from any directory instead of the
# full launcher path.  Mirrors install-claude-pmoves-command.ps1 exactly.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File deploy\provision\install-kilo-pmoves.ps1
#
# Per-node pin functions (set $env:PMOVES_NODE_ID before calling the launcher):
#   kilo-pmoves-kilocode_glm ($env:PMOVES_NODE_ID=5090)
#   kilo-pmoves-kiloclaw ($env:PMOVES_NODE_ID=5090)
# ---------------------------------------------------------------------------
param([switch]$Force)
...
$cmd = Join-Path $repoRoot "deploy\provision\kilo-pmoves.cmd"
...
if (Test-Path $shimDir) {
    $shim = Join-Path $shimDir 'kilo-pmoves.cmd'
    $q = [char]34
...
Write-Host ''
Write-Host "Installed 'kilo-pmoves' -> $cmd"
Write-Host "Also installed: 'kilo-pmoves-kilocode_glm', 'kilo-pmoves-kiloclaw'"
