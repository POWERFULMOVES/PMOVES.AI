...
# codex-pmoves.ps1 -- launch `codex` with pmoves/env.shared loaded
# ===========================================================================
# Auto-generated PowerShell twin of codex-pmoves.sh. Same shell
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File deploy\provision\codex-pmoves.ps1 [args]
#   .\codex-pmoves.cmd                     # Windows Explorer double-click
#   install-codex-pmoves-pmoves-command.ps1 ; codex-pmoves     # profile function
#
...
if (Test-Path $envf) {
    ...
    if (Test-Path $mavis_helper) {
        . $mavis_helper
        Strip-MavisSdkEnvFor -CliName 'codex'
    } else {
        Write-Warning "[codex-pmoves] mavis_sdk_env.ps1 not found at $mavis_helper -- Mavis SDK env may bleed into the launched session."
    }
    ...
    Write-Host "[codex-pmoves] loaded $n vars from $envf"
    $script:launcherSession = "codex-pmoves.ps1 ($n vars)"
} else {
    Write-Warning "[codex-pmoves] $envf not found -- MCP creds may be missing."
    $script:launcherSession = "codex-pmoves.ps1 (env file NOT FOUND)"
}
