...
# pmoves-mini.sh -- launch `pmoves-mini` with pmoves/env.shared loaded
# ===========================================================================
# Auto-generated wrapper for pmoves-mini. Mirrors the hand-written
...
#
# Usage:
#   bash deploy/provision/pmoves-mini.sh        [args...]
#   install-pmoves-mini-pmoves-command.sh && pmoves-mini         # rc function
...
if [ ! -f "${ROOT:-/nonexistent}/pmoves/Makefile" ]; then
  if [ -n "${PMOVES_ENV_SHARED:-}" ] && [ -f "$PMOVES_ENV_SHARED" ]; then
    echo "[pmoves-mini] WARN: repo root not found (${ROOT:-<unresolved>}); using PMOVES_ENV_SHARED." >&2
  else
    echo "[pmoves-mini] ERROR: no pmoves/Makefile under repo root: ${ROOT:-<unresolved>}" >&2
    echo "[pmoves-mini]        (resolved from: $SELF)" >&2
    exit 1
  fi
fi
...
if [ -f "$ENVF" ]; then
  ...
  rm -f "$tmpf"
  set -H 2>/dev/null || true
  echo "[pmoves-mini] loaded $n vars from $ENVF" >&2
else
  echo "[pmoves-mini] WARN: $ENVF not found -- MCP creds may be missing." >&2
  echo "[pmoves-mini]       run: make -C pmoves ensure-env-shared" >&2
fi
...
# --- LAUNCH ---------------------------------------------------------------
export PMOVES_LAUNCHER_SESSION="pmoves-mini.sh"
exec pmoves-mini "$@"
