> **Architecture note**: The diagrams below use the original "Central Brain / Support Systems / Muscles" three-layer model. PMOVES.AI's **Metal-Organic Framework** architecture now manifests operationally as a **rooms-on-a-stage** model — P7 (Pinokio 7) is the evolving room-aware stage manager, services are organized into rooms with stage transitions (rehearsal → live → review → archive). The topology is designed to grow. For the current model, see the [Room Manifest Contract](../pmoves/docs/ROOM_MANIFEST_CONTRACT.md) and [AGNOTE4482](../pmoves/docs/AGENTS/AGNOTE4482.md). The diagrams below are retained for historical reference.
>
