> **Architecture note**: This document uses the original "Central Brain / Support Systems / Muscles" framing. PMOVES.AI's **Metal-Organic Framework** architecture now manifests operationally as a **rooms-on-a-stage** model — P7 (Pinokio 7) is the evolving room-aware stage manager, services are organized into rooms (infra, field control, voice studio, workstation), and each room transitions through stage states (rehearsal → live → review → archive). The topology is designed to grow. For the current model, see the [Room Manifest Contract](../pmoves/docs/ROOM_MANIFEST_CONTRACT.md) and [AGNOTE4482](../pmoves/docs/AGENTS/AGNOTE4482.md). The content below is retained for historical reference.
>
