...
| **G4** | **Research seeding** — YT → Open Notebook path into the room's workspace (`workspace_ref: tokenism-demo`). | Per ingest-chain fan-out report (§ below). The room only needs transcripts to LAND in its notebook workspace; scenario-parameter extraction from transcripts is a later, separate enhancement — not demo-blocking. |
